...
func SaveSCR(filename string, bitmap []byte, bitmapSize int, packMethod PackMethod, format OutputFormat, params SCRParams, palette []uint16, mode5Colors [][]int) (int, error) {
	...
	case OutputAssembler:
		file, err := os.Create(filename)
		if err != nil {
			return 0, fmt.Errorf("failed to create ASM file: %w", err)
		}
		defer file.Close()
		...
		// Write ORG directive
		fmt.Fprintf(file, "; CPC Graphics Data - Generated by go-cpc-image\n")
		fmt.Fprintf(file, "    ORG #%04X\n\n", startAdr)

		// Write palette as DB
		fmt.Fprintf(file, "; Mode: %d\n", params.VirtualMode)
		fmt.Fprintf(file, "Palette:\n")
		fmt.Fprintf(file, "    DB %d ; mode\n", params.VirtualMode)
		for i := 0; i < 16; i++ {
			fmt.Fprintf(file, "    DB %d ; pen %d\n", palette[i], i)
		}
		fmt.Fprintf(file, "\n")

		// Write screen data
		fmt.Fprintf(file, "ScreenData:\n")
		for i := 0; i < len(dataToWrite); i += 16 {
			end := i + 16
			...
			for j := i; j < end; j++ {
				...
				fmt.Fprintf(file, "#%02X", dataToWrite[j])
			}
			fmt.Fprintf(file, "\n")
		}

	case OutputDSK:
		entete := cpc.CreeEntete("IMAGE.SCR", startAdr, uint16(lg), exec)
		headerBytes, err := cpc.AmsdosToByte(entete)
		if err != nil {
			return 0, fmt.Errorf("failed to create AMSDOS header: %w", err)
		}
		...
		fullData := make([]byte, 128+len(dataToWrite))
		copy(fullData[:128], headerBytes)
		copy(fullData[128:], dataToWrite)
		...
		result := dsk.CopieFichier(fullData, "IMAGE.SCR", len(fullData), mb, 0)
		if result != ErrNoErr {
			return 0, fmt.Errorf("failed to add file to DSK: %s", result.Error())
		}
		if err := dsk.Save(filename); err != nil {
			return 0, fmt.Errorf("failed to save DSK: %w", err)
		}
	}
